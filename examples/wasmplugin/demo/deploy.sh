...
#
# Prerequisites: setup.sh has been run (registry + port-forward up).
#
set -euo pipefail
...
info "Waiting for hot-reload (~10s)..."
sleep 12
...
epp_pod=$(kubectl --context "${KUBE_CONTEXT}" -n "${NAMESPACE}" \
    get pods -o name | grep endpoint-picker | head -1)
if [[ -n "${epp_pod}" ]]; then
    kubectl --context "${KUBE_CONTEXT}" -n "${NAMESPACE}" \
...
echo ""
info "Deployed ${TAG}. To switch: ./examples/wasmplugin/demo/deploy.sh <standard|premium|all>"
